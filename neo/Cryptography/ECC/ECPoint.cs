...
namespace Neo.Cryptography.ECC
{
    ...
    public class ECPoint : IComparable<ECPoint>, IEquatable<ECPoint>, ISerializable
    {
        ...
        /// <param name="y">An ECField object which represents the Y axis value</param>
        ...
        /// </summary>
        /// <param name="reader">A binaryReader from which it reads the ECPoint</param>
        ...
        /// <returns>If compressed , return the byte array which is using compress algorithm to encode
        ...
        /// If one of two points is zero, return <c>false</c>.<BR/>
        ...
        /// <returns>If two objects are equal then return <c>true</c>, otherwise it returns <c>false</c></returns>
        ...
        // </returns>
        // <exception cref="System.FormatException">当pubkey的字节长度不在33、65、64、72、96、104的范围内</exception>
        /// <summary>
        /// Decode a public key generated by an elliptic curve algorithm into an ECPoint object
        /// </summary>
        /// <param name="pubkey">
        /// The public key need to be decoded<br/>
        /// 1. When the byte length of pubkey is 33 bytes, it represents a complete compressed public key.<br/>
        /// 2. When the byte length of pubkey is 65 bytes, it represents a complete uncompressed public key.<br/>
        /// 3. When the byte length of pubkey is 64 bytes，the prefix identification byte (0x04) is automatically added to ,it represents a complete uncompressed public key<br/>
        /// 4. When the byte length of pubkey is 72 bytes，indicates extended format 1 (reserved)，the first 8 bytes will be omitted, and the prefix identification byte will be added automatically (0x04),it represents a complete uncompressed public key<br/>
        /// 5. When the byte length of pubkey is 96 bytes，indicates extended format 2 (reserved)，the last 32 bytes will be omitted, and the prefix identification byte will be added automatically (0x04),it represents a complete uncompressed public key<br/>
        /// 6. When the byte length of pubkey is 104 bytes，indicates extended format 3 (reserved)，the first 8 bytes and last 32 bytes will be omitted, and the prefix identification byte will be added automatically (0x04),it represents a complete uncompressed public key<br/>
        /// </param>
        /// <param name="curve">elliptic curve</param>
        /// <returns>
        ///  Decode different types of public key to ECPoint object
        /// </returns>
        /// <exception cref="System.FormatException">When the length of pubkey is not in the range of 33, 65, 64, 72, 96, 104</exception>
        ...
        /// <summary>
        /// - operator, calculate the difference between two ECPoints on the curve
        /// </summary>
        /// <param name="x">The first ECPoint</param>
        /// <param name="y">The second ECPoint</param>
        /// <returns>return the difference between two ECPoint. If the second point is Infinity Point return the first Point. Otherwise return the sum of the first point and the negative one of second point</returns>
        public static ECPoint operator -(ECPoint x, ECPoint y)
        {
            if (y.IsInfinity)
                return x;
